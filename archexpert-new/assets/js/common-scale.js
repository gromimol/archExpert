...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (formType === 'checklist') {
            window.location.href = '/page-success-lm/';
        } else {
...
window.initThreeJsParticles = function initThreeJsParticles() {
...
// Вызывается из index.html после загрузки Three.js

// ===== FINAL CTA WEBGL BACKGROUND FOR `.final-cta__bg` =====
window.initFinalCtaWebGL = function initFinalCtaWebGL() {
...
// ===== BENEFITS CARDS AUTO SCROLL =====
document.addEventListener('DOMContentLoaded', function() {
    const cardsContainer = document.querySelector('.benefits__cards');
    if (!cardsContainer) return;

    const cards = Array.from(cardsContainer.querySelectorAll('.benefits__card'));
    if (cards.length === 0) return;

    // Клонируем карточки для бесшовного зацикливания
    const clonedCards = cards.map(card => card.cloneNode(true));
    clonedCards.forEach(clone => cardsContainer.appendChild(clone));

    let scrollSpeed = 0.5; // пикселей за кадр
    let isHovered = false;
    ...
    function autoScroll() {
        if (!isHovered && !isDragging) {
            ...
            // Когда доходим до конца первой половины (оригинальных карточек),
            // возвращаемся в начало для бесшовного цикла
            const maxScroll = cardsContainer.scrollWidth / 2;
